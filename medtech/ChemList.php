...
$data = $lab->fetchlabByDate("lab_chemistry",$month,$year);
					...
					<tr>
							...
							<td > 
								<?php 
									...
									$tid = $transID[0];
									$count = $lab->checkPrint($pid, $tid, 'CHEMISTRY');
									if ($count > 0) {
										$class = "btn-warning";
									}else{
										$class = "btn-info";
								...
								?>
								<button type="button" class="btn <?php echo $class; ?>" onclick="document.location = 'LabChemFORM.php?id=<?php echo $patient['PatientID']."&tid=".$transID?>';">View Certificate</button>
							</td>
                ...
                buttons: [
                    {
                        ...
                        action: function ( e, dt, node, config ) {
						   ...
						   "ChemList.php?month=1&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'February',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=2&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'March',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=3&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'April',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=4&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'May',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=5&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'June',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=6&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'July',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=7&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'August',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=8&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'September',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=9&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'October',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=10&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'November',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=11&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'December',
                        action: function ( e, dt, node, config ) {
						   window.location.href = 
						   "ChemList.php?month=12&year=<?php echo $year ?>";
                        }
            		}
            		]
           }
