...
<center><p>LABORATORY SEROLOGY AND TOXICOLOGY</p></center>
	<table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
					...
					<?php foreach  ($patients as $patient) { 
						...
						$data = $lab->getData($pid, $tid, "lab_serology");
						$data2 = $lab->getData($pid, $tid, "lab_toxicology");
					?>
					...
					<tr>
							...
							<td nowrap> 
								<?php if(is_array($data) or is_array($data2)){ ?>
								<button type="button" class="btn btn-success" onclick="document.location = 'LabSTVIEW.php?id=<?php echo $patient['PatientID']?>&tid=<?php echo $patient['TransactionID']?>';">VIEW RECORD</button>
								<?php }else{ ?>
								<button type="button" class="btn btn-primary" onclick="document.location = 'LabSeroToxiADD.php?id=<?php echo $patient['PatientID']?>&tid=<?php echo $patient['TransactionID']?>';">ADD RECORD</button>
								<?php } ?>
							</td>
                ...
                buttons: [
                    {
                        ...
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=1&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'February',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=2&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'March',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=3&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'April',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=4&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'May',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=5&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'June',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=6&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'July',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=7&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'August',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=8&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'September',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=9&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'October',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=10&year=<?php echo $year ?>";
                        }
                    },
                    {
                        text: 'November',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=11&year=<?php echo $year ?>";
                        }
            		},
            		{
                        text: 'December',
                        action: function ( e, dt, node, config ) {
                           window.location.href = "LabSeroToxi.php?month=12&year=<?php echo $year ?>";
                        }
            		}
            		]
           }
